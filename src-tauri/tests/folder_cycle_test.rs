...
use omnivault_lib::db::storage::{create_folder, create_item, delete_folder, list_folders, move_folder};
...
/// A folder must never become its own ancestor. The guard existed only in the
/// TypeScript layer, so the Tauri command and `/api/folders/move` could both
/// create a cycle, which then makes the recursive descendant walk in
/// delete_folder unbounded.
#[test]
fn a_folder_cannot_be_moved_inside_its_own_descendant() {
    ...
    assert!(res.is_err(), "moving a folder into its own child must be rejected");
    ...
    let parent_now = list_folders(&c, false)
        ...
        .unwrap();
    assert_eq!(parent_now.parent_id, None, "the rejected move must not have been applied");
}
...
fn deleting_a_deep_tree_terminates_and_rescues_items() {
    ...
    assert_eq!(list_folders(&c, false).unwrap().len(), 0, "the whole subtree is deleted");
    let n = omnivault_lib::db::storage::get_item_by_id(&c, &note.id).unwrap().unwrap();
    assert!(!n.is_deleted && n.folder_id.is_none(), "the deep note is rescued to the inbox");
}
